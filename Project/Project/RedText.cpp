...
{
	static RedText instance;
	return &instance;
}

void RedText::Initialize(UINT texnumber)
{
	// 全てのスプライトデータについて
	for (int i = 0; i < _countof(spriteDatas); i++)
	{
...
// 1文字列追加
void RedText::Print(const std::string & text, float x, float y, float scale = 1.0f)
{
	SetPos(x, y);
	SetSize(scale);
...
void RedText::NPrint(int len, const char* text)
{
	// 全ての文字について
	for (int i = 0; i < len; i++)
	{
...
void RedText::Printf(const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
// まとめて描画
void RedText::DrawAll(ID3D12GraphicsCommandList* cmdList)
{
	// 全ての文字のスプライトについて
